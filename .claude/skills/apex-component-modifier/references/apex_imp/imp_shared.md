...
Source: APEX 24.2 (`APEX_240200`). Active package for shared component imports.

Note: Some shared components (LOVs, lists, shortcuts, app processes) still use the frozen `wwv_flow_imp` package. Newer ones (build options, auth, UI, static files) use `wwv_flow_imp_shared`. All IDs should be wrapped in `wwv_flow_imp.id()`.
...
procedure create_user_interface (
    ...
    -- also: p_flow_id, p_theme_style_by_user_pref
    );
```
...
SQL-based LOV (via `wwv_flow_imp`, frozen):
```sql
...
Static LOV (via `wwv_flow_imp_shared`):
```sql
...
### Authorization (create_security_scheme)
...
function create_static_file (
    ...
    p_mime_type in varchar2, p_file_content in blob,
    -- also: p_id, p_flow_id, p_plugin_id, p_theme_id, p_file_charset
    ) return number;
```
